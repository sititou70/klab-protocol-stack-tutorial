...
static struct arp_cache *arp_cache_update(ip_addr_t pa, const uint8_t *ha) {
  ...
  if (!cache) {
    errorf("arp_cache_update: cache not found, pa=%s, ha=%s", ip_addr_ntop(pa, addr1, sizeof(addr1)),
  ...
  if (gettimeofday(&cache->timestamp, NULL)) {
    errorf("arp_cache_update: gettimeofday failed");
    return NULL;
  };
...
static void arp_input(const uint8_t *data, size_t len, struct net_device *dev) {
  ...
  /* try update arp cache */
  int arp_cache_updated = 0;
  mutex_lock(&mutex);
