...
int sched_ctx_destroy(struct sched_ctx *ctx) { return pthread_cond_destroy(&ctx->cond); }
...
int sched_wakeup(struct sched_ctx *ctx) { return pthread_cond_broadcast(&ctx->cond); }
